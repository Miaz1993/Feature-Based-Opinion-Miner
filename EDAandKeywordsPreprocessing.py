...
# Read data
engagement1 = pd.read_excel('filename.xlsx')
engagement2 = pd.read_excel('filename.xlsx')
engagement3 = pd.read_excel('filename.xlsx')
engagement = pd.concat([engagemen1,engagement2,engagement3], keys=['Comment Text'])
engagement.head()
...
##bi&trigram
#lst = []
#for sentence in engagement_eng['Comment Text'].values:
#  sentence = re.sub('[^A-Za-z0-9\s]+', '', sentence).lower()
#  sentence = word_tokenize(sentence)
#  lemma=WordNetLemmatizer()
#  sentence_raw=[lemma.lemmatize(te) for te in sentence]
#  sentence_final = ' '.join(sentence_raw)
#  lst.append(sentence_final)
#
#engagement_eng['gram_comment'] = lst
...
def wordfreq(words,z):
  ...
  result.plot.bar(rot=1,color='royalblue',figsize=(15,7))
  plt.grid(False)
  plt.xticks()
...
EMB_DIM = 50
w2v = Word2Vec(clean,size=EMB_DIM,min_count=10,window=2,workers=multiprocessing.cpu_count())
vectors=w2v.wv
key_w2v = set([vectors.similar_by_word(word) for word in keywords])
