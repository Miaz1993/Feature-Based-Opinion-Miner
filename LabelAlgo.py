...
#      print(sentence_raw)
#      print(sentence_raw[i:min(i+3,len(sentence_raw))])
#      gram_num,gram = self.is_gram(sentence_raw[i:min(i+3,len(sentence_raw))])
...
#
#  sen = 'le spreadsheet and a different operating system sap is more complex than a system need to be and is not user friendly at all a lot of time and money are wasted because of how complex it is there are constantly problem with it using a different operating system could cut time and personnel in half beside the fact that no one is ever trained to use it'
